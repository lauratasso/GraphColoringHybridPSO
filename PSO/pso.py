    ...
    def run(self):
        ...
        for t in range(self.iterations):
            ...
            for i, particle in enumerate(self.particles):
                ...
                if  self.graph.eval_fitness(new_position) < self.graph.eval_fitness(particle): #nova posicao < pBest
                    self.particles[particle] = new_position

                if self.graph.eval_fitness(new_position) < self.graph.eval_fitness(self.particles[gBest]): #nova pos < gBest
                    gBest = i
        ...
        result = min(self.betterThanAll)
        self.graph.transform_to_nx_graph()
        return result
...
if __name__ == "__main__":
    ...
    print('Quantidade minima de cores : ', pso.run())
    stop = time.time()
    print('Tempo gasto: ', str(stop-start), 'segundos')
